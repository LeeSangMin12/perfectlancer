						...
						<div class="flex items-start justify-between gap-2">
							<div class="flex min-w-0 flex-1 flex-col gap-1">
								{#if n.type === 'post.liked'}
									...
									<p class="mt-1 text-sm break-words text-gray-600">
								...
								{:else if n.type === 'gift.received'}
									<p class="text-sm">
										선물을 받았습니다. (+{n.payload?.amount} 포인트)
									</p>
								{:else if n.type === 'inquiry.requested'}
									<p class="text-sm">
										<strong
											>{n.actor?.name ||
												'@' + (n.actor?.handle || '익명')}</strong
								...
								{:else if n.type === 'inquiry.accepted'}
									<p class="text-sm">
										<strong
											>{n.actor?.name ||
												'@' + (n.actor?.handle || '익명')}</strong
								...
								{:else if n.type === 'inquiry.rejected'}
									<p class="text-sm">
										<strong
											>{n.actor?.name ||
												'@' + (n.actor?.handle || '익명')}</strong
									...
									<p class="mt-1 text-sm break-words text-gray-600">
										...
										{#if n.payload?.payout}
											<span class="text-blue-600"
												>(+{n.payload.payout.toLocaleString()}원)</span
											>
										{/if}
									</p>
							...
							<div
								class="shrink-0 self-start text-xs whitespace-nowrap text-gray-500"
							>
