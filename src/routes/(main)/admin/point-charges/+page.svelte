<script>
	...
	let all_charges = $state(data.recent_charges || []);
	...
	const handle_approve = async () => {
		...
		try {
			await api.point_charges.approve(selected_charge.id, me.id);
			show_toast('충전 요청이 승인되었습니다.', 'success');

			is_approve_modal_open = false;

			// 데이터 업데이트
			pending_charges = pending_charges.filter((c) => c.id !== selected_charge.id);
			all_charges = all_charges.map((c) =>
				c.id === selected_charge.id ? { ...c, status: 'approved' } : c,
			);
	...
	const handle_reject = async () => {
		...
		try {
			await api.point_charges.reject(selected_charge.id, me.id, reject_reason);
			show_toast('충전 요청이 거절되었습니다.', 'success');
			...
			pending_charges = pending_charges.filter((c) => c.id !== selected_charge.id);
			all_charges = all_charges.map((c) =>
				c.id === selected_charge.id ? { ...c, status: 'rejected', reject_reason } : c,
			);
			selected_charge = null;
			reject_reason = '';
		} catch (error) {
	...
	class AmountRenderer {
		...
		init(params) {
			this.eGui = document.createElement('span');
			this.eGui.className = 'font-medium text-blue-600';
	...
	class BankAccountRenderer {
		...
		init(params) {
			this.eGui = document.createElement('div');
			const { bank, account_number } = params.data;
			if (bank && account_number) {
				...
				this.eGui.innerHTML = `<span class="text-gray-400 text-sm">-</span>`;
			}
		}
	...
	// 현재 데이터 및 컬럼
	let current_data = $derived(view_mode === 'pending' ? pending_charges : all_charges);
	let current_columns = $derived(view_mode === 'pending' ? pending_column_defs : all_column_defs);
</script>

<svelte:head>
	<title>포인트 충전 관리 | 관리자</title>
</svelte:head>
...
<Header>
	...
	{/snippet}
	{#snippet center()}
		<h1 class="font-semibold">포인트 충전 관리</h1>
	{/snippet}
</Header>
	...
	<div class="mb-6 flex gap-4">
		<div class="rounded-lg border border-gray-200 bg-white px-4 py-3">
			...
			<p class="text-xl font-semibold">
				{pending_charges.length}<span class="text-sm font-normal text-gray-400">건</span>
			</p>
		</div>
		<div class="rounded-lg border border-gray-200 bg-white px-4 py-3">
			<p class="text-xs text-gray-500">대기 금액</p>
			<p class="text-xl font-semibold">
				{comma(pending_charges.reduce((sum, c) => sum + c.amount, 0))}<span
		...
		<div class="rounded-lg border border-gray-200 bg-white px-4 py-3">
			...
			<p class="text-xl font-semibold">
				{all_charges.length}<span class="text-sm font-normal text-gray-400">건</span>
			</p>
		</div>
	</div>
	...
	<div class="rounded-lg border border-gray-200 bg-white">
		{#if current_data.length === 0}
			<div class="py-16 text-center text-gray-400">
				{view_mode === 'pending' ? '승인 대기 중인 충전 요청이 없습니다.' : '충전 내역이 없습니다.'}
			</div>
		{:else}
			{#key view_mode}
				<AgGrid
...
<Modal bind:is_modal_open={is_approve_modal_open} modal_position="center">
	<div class="p-5">
		<h3 class="mb-4 font-semibold text-gray-900">충전 요청 승인</h3>
		{#if selected_charge}
			<div class="rounded-lg bg-gray-50 p-3">
				<p class="text-sm font-medium text-gray-900">
					{selected_charge.users?.name}님
				</p>
				<p class="mt-1 text-lg font-semibold text-blue-600">
					+{comma(selected_charge.amount)}원
				</p>
				<p class="mt-1 text-sm text-gray-500">
					입금자명: {selected_charge.depositor_name}
				</p>
				{#if selected_charge.bank && selected_charge.account_number}
					<p class="mt-1 text-sm text-gray-500">
						입금 계좌: {selected_charge.bank} {selected_charge.account_number}
					</p>
				{/if}
			</div>
		{/if}
...
<Modal bind:is_modal_open={is_reject_modal_open} modal_position="center">
	<div class="p-5">
		<h3 class="mb-4 font-semibold text-gray-900">충전 요청 거절</h3>
		{#if selected_charge}
