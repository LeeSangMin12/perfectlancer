<script>
	...
	let { data } = $props();
	let { work_request, my_proposal, my_templates, user } = $state(data);

	// 뷰 모드: 'form' | 'template_list' | 'preview'
	let view_mode = $state('form');

	// 제안 메시지
	let message = $state(my_proposal.message || '');

	// 견적서 사용 여부
	let use_quote = $state(!!my_proposal.quote_data);
	let selected_template_id = $state(my_proposal.quote_template_id || null);

	// 기존 견적서 데이터로 초기화
	const init_quote_form = () => {
		if (my_proposal.quote_data) {
			return {
				title: my_proposal.quote_data.title || '',
				description: my_proposal.quote_data.description || '',
				price: my_proposal.quote_data.price
					? my_proposal.quote_data.price.toLocaleString()
					: '',
				duration: my_proposal.quote_data.duration || '',
				target_audience: my_proposal.quote_data.target_audience || '',
				work_process:
					Array.isArray(my_proposal.quote_data.work_process) &&
					my_proposal.quote_data.work_process.length > 0
						? my_proposal.quote_data.work_process
						: [{ title: '', description: '' }],
				deliverables:
					Array.isArray(my_proposal.quote_data.deliverables) &&
					my_proposal.quote_data.deliverables.length > 0
						? my_proposal.quote_data.deliverables
						: [''],
				includes:
					Array.isArray(my_proposal.quote_data.includes) &&
					my_proposal.quote_data.includes.length > 0
						? my_proposal.quote_data.includes
						: [''],
				revision_policy: my_proposal.quote_data.revision_policy || '',
				refund_policy: my_proposal.quote_data.refund_policy || '',
				faq:
					Array.isArray(my_proposal.quote_data.faq) &&
					my_proposal.quote_data.faq.length > 0
						? my_proposal.quote_data.faq
						: [{ question: '', answer: '' }],
			};
		}
		return {
	...
	let quote_form = $state(init_quote_form());
	...
	// 제안 삭제
	const delete_proposal = async () => {
		update_global_store('loading', true);
		try {
			await api.work_request_proposals.delete(my_proposal.id, user.id);
			show_toast('success', '제안이 삭제되었습니다.');
			goto(`/work-request/${work_request.id}`, { replaceState: true });
		} catch (error) {
			console.error('Delete error:', error);
			show_toast('error', error.message || '삭제 중 오류가 발생했습니다.');
		} finally {
			update_global_store('loading', false);
...
<svelte:head>
	<title>제안 수정 | {work_request.title}</title>
</svelte:head>
...
{:else if view_mode === 'preview'}
	...
	<main class="min-h-screen bg-gray-50 pb-32">
		...
		<div class="bg-white px-5 py-5">
			<p class="mb-2 text-[13px] font-medium text-gray-400">제안 메시지</p>
			<p class="text-[15px] leading-relaxed whitespace-pre-wrap text-gray-800">
				{message || '(메시지 없음)'}
			</p>
		</div>
		...
		{#if use_quote}
			...
			{#if quote_form.description && quote_form.description !== '<p></p>'}
				<div class="border-t border-gray-100 bg-white px-5 py-6">
					<p class="mb-3 text-[13px] font-medium text-gray-400">서비스 설명</p>
					<div
			...
			{#if quote_form.target_audience}
				<div class="border-t border-gray-100 bg-white px-5 py-6">
					<p class="mb-3 text-[13px] font-medium text-gray-400">
						이런 분께 추천드립니다
					</p>
					<p
						class="text-[15px] leading-relaxed whitespace-pre-wrap text-gray-800"
					>
			...
			{#if preview_steps.length > 0}
				<div class="border-t border-gray-100 bg-white px-5 py-6">
					<p class="mb-5 text-[13px] font-medium text-gray-400">
						작업 프로세스
					</p>
			...
			{#if preview_includes.length > 0}
				<div class="border-t border-gray-100 bg-white px-5 py-6">
					<p class="mb-3 text-[13px] font-medium text-gray-400">포함 사항</p>
			...
			{#if quote_form.revision_policy || quote_form.refund_policy}
				<div class="space-y-5 border-t border-gray-100 bg-white px-5 py-6">
			...
			{#if preview_faq.length > 0}
				<div class="border-t border-gray-100 bg-white px-5 py-6">
					<p class="mb-4 text-[13px] font-medium text-gray-400">
						자주 묻는 질문
					</p>
					<div class="space-y-4">
	...
	<main class="space-y-8 px-4 py-6 pb-28">
		...
		<!-- <section class="border-t border-gray-100 pt-6">
			<button
				...
				class="w-full text-center text-sm text-gray-400 hover:text-red-500"
			>
				제안 삭제하기
			</button>
		</section> -->
...
<Modal bind:is_modal_open={show_delete_modal} modal_position="center">
	<div class="p-5">
		<p class="text-[16px] font-semibold text-gray-900">제안을 삭제할까요?</p>
