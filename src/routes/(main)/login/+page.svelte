	...
	const email_login = async () => {
		...
		try {
			...
			const { exists, provider } =
				await api.auth.check_email_provider(email_login_id);
	...
	<div class="p-5 px-6">
		...
		<div class="flex flex-col gap-4">
			...
			<div class="pb-safe mt-2 flex flex-col gap-3">
				<button
					class="h-[52px] w-full rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 text-base font-semibold text-white transition-all hover:-translate-y-0.5 active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:translate-y-0"
					onclick={email_login}
				...
				<div class="flex items-center justify-between text-sm">
					<a
						href="/sign-up/email"
						class="text-slate-500 transition-colors hover:text-blue-500"
